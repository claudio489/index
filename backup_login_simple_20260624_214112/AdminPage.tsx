...
// ADMIN - 100% compatible con login existente
// Usa IndexedDB + localStorage fallback (igual que useSessionStore)
// Guarda en DB 'index-auth' store 'codes' + localStorage 'index_codes'
// PIN: 458555 (hash corregido a SHA256 real)
...
// CONFIGURACION
// ============================================
const MASTER_PIN_HASH = '9c7b8a341dd477eebe11dce4e2a70446f700f59d3177d22d237cb6b58f920be3'; // SHA256('458555') - CORREGIDO
const CODE_PREFIX = 'INDEX-';

// ============================================
// TYPES (identicos a accessControl.ts)
// ============================================
interface AccessCode {
  id: string;
  name: string;
  code: string;        // Codigo INDEX-WXYZ visible
  codeHash: string;
  courses: string[];
  expiresAt: string;
  createdAt: string;
...
// ============================================
// INDEXEDDB (copiado de useSessionStore)
// ============================================
...
async function saveCodeToDB(code: AccessCode) {
  try {
...
async function deleteCodeFromDB(id: string) {
  try {
    const db = await getDB();
    const tx = db.transaction('codes', 'readwrite');
...
// ============================================
// LOCALSTORAGE (fallback, igual que useSessionStore)
// ============================================
...
async function loadAllCodes(): Promise<AccessCode[]> {
  // Intentar DB primero, luego LS
  const dbCodes = await getCodesFromDB();
  const lsCodes = getCodesFromLS();

  // Merge: DB tiene prioridad, pero incluir los de LS que no esten en DB
  const merged = [...dbCodes];
  for (const ls of lsCodes) {
    if (!merged.find(c => c.id === ls.id)) {
      merged.push(ls);
      // Sync a DB
      await saveCodeToDB(ls);
    }
  }
  return merged;
}

async function migrateCodesToIndexFormat(codes: AccessCode[]): Promise<AccessCode[]> {
  let migrated = false;
  const updated = codes.map(c => {
    if (!c.code || c.code === "") {
      migrated = true;
      const suffix = c.id.replace("code-", "").slice(-6).toUpperCase().replace(/[^A-Z0-9]/g, "").padEnd(4, "0").slice(0, 4);
      return { ...c, code: `INDEX-${suffix}` };
    }
    return c;
  });
  if (migrated) {
    saveCodesToLS(updated);
    for (const c of updated) await saveCodeToDB(c);
  }
  return updated;
}
...
async function saveCode(code: AccessCode) {
  ...
  if (idx >= 0) existing[idx] = code;
  else existing.push(code);
  saveCodesToLS(existing);
}

async function deleteCode(id: string) {
  await deleteCodeFromDB(id);
  const existing = getCodesFromLS().filter((c: AccessCode) => c.id !== id);
  saveCodesToLS(existing);
  // Tambien borrar device binding si existe
  const bindings = JSON.parse(localStorage.getItem('index_device_bindings') || '[]');
  const filtered = bindings.filter((b: any) => b.codeId !== id);
  localStorage.setItem('index_device_bindings', JSON.stringify(filtered));
}
      ...
      <div className="max-w-sm w-full">
        <div className="bg-slate-900 rounded-2xl p-8 border border-slate-800 shadow-2xl">
          ...
          <p className="text-center text-xs text-slate-600 mt-6">
            Dive Tools.cl - PADI TEC/REC INSTRUCTOR #458555
          </p>
        </div>
      </div>
    </div>
...
function AdminDashboard({ onLogout }: { onLogout: () => void }) {
  ...
  useEffect(() => {
    loadAllCodes().then(codes => migrateCodesToIndexFormat(codes)).then(setCodes);
  }, []);

  const handleGenerate = async () => {
    const rawCode = generateAccessCode(); // ej: INDEX-AB3D9F
    const codeHash = hash(rawCode.toUpperCase()); // SHA256 para validacion (uppercase como el login)

    const newCode: AccessCode = {
      id: `code-${Date.now()}`,
      name: newName || 'Sin nombre',
      code: rawCode,                     // Codigo INDEX-WXYZ visible
      codeHash: codeHash,                 // Hash SHA256 (asi lo valida el login)
      courses: newCourses,
    ...
    setCodes(updated);
    setGeneratedCode(rawCode);             // Mostramos el codigo plano al admin
    setNewName('');
    setNewCourses([]);
    setNewNotes('');
  };
      ...
      <main className="max-w-4xl mx-auto p-4 space-y-6">
        ...
        <div className="bg-slate-900 rounded-xl border border-slate-800 overflow-hidden">
          ...
          >
            ...
            <span className="text-slate-400">{showNew ? 'âˆ’' : '+'}</span>
          </button>
        ...
        <div className="space-y-3">
          ...
          {codes.map(code => {
            ...
            return (
              <div
                key={code.id}
                className={`bg-slate-900 rounded-xl p-4 border ${expired ? 'border-red-500/30' : 'border-slate-800'} space-y-3`}
              >
                <div className="flex items-start justify-between">
                  <div>
                    ...
                    <p className="text-xs text-slate-500 font-mono mt-1">ID: {code.id}</p>
                  </div>
                ...
                <div className="flex flex-wrap gap-1">
                  {code.courses.map(c => (
                    <span key={c} className="text-xs bg-slate-800 text-slate-300 px-2 py-1 rounded-full">{c}</span>
                  ))}
                </div>
                ...
                <div className="flex items-center gap-4 text-xs text-slate-500">
                  <span className="flex items-center gap-1">
                    ...
                    Expira: {new Date(code.expiresAt).toLocaleDateString('es-CL')}
                  </span>
                  <span>Creado: {new Date(code.createdAt).toLocaleDateString('es-CL')}</span>
                </div>

                {code.notes && (
                  <p className="text-xs text-slate-400 bg-slate-800/50 p-2 rounded-lg">{code.notes}</p>
                )}
              </div>
            );
          })}
