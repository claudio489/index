# fix-v9.ps1 - Solo patch en accessControl.ts + buhlmann.ts funcional
...
Write-Host "  Fix v9: codeHash opcional + buhlmann.ts" -ForegroundColor Cyan
Write-Host "========================================" -ForegroundColor Cyan
Write-Host ""

# ---- Step 1: Restore accessControl.ts + patch codeHash opcional ----
...
if ($acBackup) {
    Write-Host "[1/3] Restoring accessControl.ts + patch codeHash opcional..." -ForegroundColor Yellow
    Copy-Item $acBackup.FullName $acPath -Force
    $content = Get-Content $acPath -Raw
    # Hacer codeHash opcional en encryptCode y decryptCode
    $content = $content -replace "export function encryptCode\(plainCode: string, codeHash: string\): string", "export function encryptCode(plainCode: string, codeHash?: string): string"
    $content = $content -replace "export function decryptCode\(encrypted: string, codeHash: string\): string", "export function decryptCode(encrypted: string, codeHash?: string): string | null"
    # Asegurar que encryptCode maneje codeHash undefined
    if ($content -match "const hash = codeHash \\|\\| generateCodeHash") {
        Write-Host "  Ya tiene fallback de hash" -ForegroundColor Green
    } else {
        $content = $content -replace "(const hash = )(codeHash)", "$1$2 || generateCodeHash(plainCode)"
    }
    # Asegurar que decryptCode maneje codeHash undefined
    if ($content -match "codeHash\\?\\.") {
        Write-Host "  Ya maneja codeHash opcional" -ForegroundColor Green
    }
    Set-Content $acPath $content -Encoding UTF8 -NoNewline
    Write-Host "  Patched: codeHash es opcional" -ForegroundColor Green
} else {
    Write-Host "  ERROR: No backup de accessControl.ts" -ForegroundColor Red
    exit 1
}

# ---- Step 2: Write buhlmann.ts ----
Write-Host "[2/3] Writing buhlmann.ts (ZHL-16C funcional)..." -ForegroundColor Yellow
$bhB64 = @(
...
# ---- Step 3: Restore hooks/useSessionStore.ts ----
...
if ($stBackup) {
    Write-Host "[3/3] Restoring hooks/useSessionStore.ts..." -ForegroundColor Yellow
    Copy-Item $stBackup.FullName $stPath -Force
    Write-Host "  Restored: $($stBackup.Name)" -ForegroundColor Green
} else {
    Write-Host "  No backup de hooks/useSessionStore.ts" -ForegroundColor Yellow
}
